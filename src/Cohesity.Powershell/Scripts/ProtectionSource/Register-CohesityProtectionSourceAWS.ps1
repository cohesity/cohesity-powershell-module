function Register-CohesityProtectionSourceAWS {
  <#
        ...
        .EXAMPLE
        Register-CohesityProtectionSourceAWS -AccessKey "access-key" -SecretAccessKey "secret-key" -ARN "aws-arn" -FleetSubnetType kSourceVM -FleetTags $tags -NetworkParamsList $networkList
        Construct the network list as follows,
        $networkList = @()
        $networkList += @{region="US";subnet="255.255.255.0";vpc="theVPC"}
  ...
  Param(
    ...
    [object[]]$FleetTags,
    [Parameter(Mandatory = $false)]
    # Fleet settings, network list.
    [object[]]$NetworkParamsList
  )
  ...
  Process {
    ...
    if ($FleetSubnetType) {
        $awsFleetParams = @{
            ...
            fleetTags = $FleetTags
            networkParamsList = $NetworkParamsList
        }
