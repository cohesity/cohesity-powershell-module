...
if ($forceRefresh) {
    $refreshOutput = (Get-CohesityProtectionSource -Environments KVMware).protectionSource | Update-CohesityProtectionSource
...
foreach ($currentJob in $cohesityJobs) {
    ...
    $property = Get-Member -InputObject $currentJob -Name ExcludeSourceIds
    if(-not $property) {
        $currentJob | Add-Member -NotePropertyName ExcludeSourceIds -NotePropertyValue @()
    }
    ...
    try {
        ...
        $currentJob | Set-CohesityProtectionJob -Confirm:$false
